...
int loadClassList(Student *classList, string fileName)
{
    ...
    {
        fin >> classList[i].fname[i] >> classList[i].lname[i];
        for(int n = 0; classList[n].grades[n] == SIZE; n++)
        {
            fin >> classList[n].grades[n];
            if(classList[n].grades == 0)
            {
                break;
            }
        }
...
void printClassList(Student *classList, int classSize)
{
    ...
    {
        cout << classList[i].fname << " " << classList[i].lname << " ";
        if(classList[i].grades[i] > 0)     
        {
            cout << classList[i].grades[i];
        }
    }
}
