...
int loadClassList(Student *classList, string fileName)
{
    ...
    while(!fin.eof())
    {
        fin >> classList[i].fname; //read first name from file
        fin >> classList[i].lname; //read last name from file
        fin >> classList[i].numGrades;  //read numGrades from file
        for(int n = 0; n < classList[i].numGrades; n++)
        {
            fin >> classList[i].grades[n]; //reads grades into an array of specific student
        }
        i++;
    }
...
{
    int count = 1;
    for (int i = 0; i < classSize; i++)//For loop to sort through array classList
    {
        cout << count++ << ". " << classList[i].fname << " " << classList[i].lname << ":" << setw(5); //cout first/name, add formatting 
        for(int n = 0; n < classList[i].numGrades; n++) //for loop to sort through grades of classList[]
        {
            cout << classList[i].grades[n] << " "; //cout grades
        }
        cout << endl;
    }
}
